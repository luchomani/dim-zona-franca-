...
# LISTA OFICIAL DE COLUMNAS (Incluyendo Casillas 76 y 77)
COLUMNAS = [
    ...
    "Código de Embalaje",
    "No. Bultos",
    "Valor Moneda Negociación (Casilla 76)",
    "Valor Fletes/Seguros/Otros (Casilla 77)",
    "Valor FOB (USD)",
...
def dividir_dims(texto: str):
  partes = re.split(
      r"(?=Declaraci[oó]n de Importaci[oó]n)", texto, flags=re.IGNORECASE
  )
  return [p for p in partes if re.search(r"N[uú]mero de formulario", p, re.IGNORECASE)]


def extraer_campos_dim(
    chunk_texto: str, texto_completo: str, nombre_archivo: str
) -> dict:
  faltantes = []
  ...
  nit_importador = campo(
      "NIT Importador",
      r"5\s*\.\s*N[uú]mero de Identificaci[oó]n Tributaria \(NIT\)\s*(\d{9,10})",
  )
  razon_social = campo(
      "Razón Social Importador",
  ...
  manifiesto_carga = campo(
      "Manifiesto de carga",
      r"42\s*\.?\s*Manifiesto\s+de\s+carga\s*(?:No\.?\s*)?([A-Za-z0-9\-]+)",
  )
  if not manifiesto_carga:
    manifiesto_carga = campo(
        "Manifiesto de carga",
        (
            r"42\s*\.?\s*Manifiesto\s+de\s+carga[^\n]*\n\s*(?:No\.?\s*)?([A-Za-z0-9\-]+)"
        ),
    )

  documento_transporte = campo(
      "Documento de transporte",
      r"44\s*\.?\s*Documento\s+de\s+transporte\s*(?:No\.?\s*)?([A-Za-z0-9\-]+)",
  )
  if not documento_transporte:
    documento_transporte = campo(
        "Documento de transporte",
        (
            r"44\s*\.?\s*Documento\s+de\s+transporte[^\n]*\n\s*(?:No\.?\s*)?([A-Za-z0-9\-]+)"
        ),
    )

  cod_pais_procedencia = campo(
      "Cod. País Procedencia",
      r"53\s*\.\s*(?:C[oó]d\.?\s*)?pa[ií]s\s+(?:de\s+)?procedencia\s*([A-Za-z0-9]{2,3})",
  )
  cod_modo_transporte = campo(
      "Cod. Modo Transporte", r"54\s*\.\s*Cod\.\s*Modo\s*Transporte\s*(\d)"
  )
  codigo_bandera = campo(
      "Código de Bandera",
      r"55\s*\.\s*C[oó]digo\s+(?:de\s+)?bandera\s*([A-Za-z0-9]{2,3})",
  )
  tasa_cambio = campo(
      "Tasa de Cambio", r"Tasa de cambio\s*\$?\s*cvs\.?\s*\n?\s*([\d.,]+)"
  )
  subpartida = campo(
      "Subpartida Arancelaria", r"59\s*\.\s*Subpartida arancelaria\s*(\d{10})"
  )
  cod_pais_origen = campo(
      "Cod. País Origen",
      r"66\s*\.\s*(?:C[oó]d\.?\s*)?pa[ií]s\s+(?:de\s+)?origen\s*([A-Za-z0-9]{2,3})",
  )
  cod_pais_compra = campo(
      "Cod. País Compra",
      r"70\s*\.\s*Cod\s*\.\s*pa[ií]s\s*\n?\s*compra\s*(\d{2,3})",
  )
  ...
  peso_bruto = limpiar_monto(
      campo(
          "Peso Bruto (Kgs)",
          r"71\s*\.\s*Peso bruto kgs\.\s*dcms\.\s*(" + MONTO + ")",
      )
  )
  peso_neto = limpiar_monto(
      campo(
          "Peso Neto (Kgs)",
  ...
  # Extracción de Casillas 76 y 77 añadidas
  casilla_76_str = campo(
      "Valor Moneda Negociación (Casilla 76)",
      r"76\s*\.\s*[^\n]*\n\s*(" + MONTO + ")",
  )
  if not casilla_76_str:
    casilla_76_str = campo(
        "Valor Moneda Negociación (Casilla 76)",
        r"76\s*\.\s*.*?\s*(" + MONTO + ")",
    )
  valor_casilla_76 = limpiar_monto(casilla_76_str)

  casilla_77_str = campo(
      "Valor Fletes/Seguros/Otros (Casilla 77)",
      r"77\s*\.\s*[^\n]*\n\s*(" + MONTO + ")",
  )
  if not casilla_77_str:
    casilla_77_str = campo(
        "Valor Fletes/Seguros/Otros (Casilla 77)",
        r"77\s*\.\s*.*?\s*(" + MONTO + ")",
    )
  valor_casilla_77 = limpiar_monto(casilla_77_str)

  valor_fob = limpiar_monto(
      campo("Valor FOB (USD)", r"78\s*\.\s*Valor FOB USD\s*(" + MONTO + ")")
  )
  sumatoria_fletes = limpiar_monto(
      campo(
          "Sumatoria Fletes/Seguros/Otros (USD)",
          r"82\s*\.\s*Sumatoria de fletes,?\s*seguros\s*\n?\s*y otros gastos USD\s*("
          + MONTO
          + ")",
      )
  )
  ...
  try:
    no_bultos = (
        int(re.sub(r"[^\d]", "", n_bultos_str)) if n_bultos_str else 0
    )
  except ValueError:
    no_bultos = 0
  ...
  levante_no = ""
  m_lev_box = re.search(
      r"134\.?\s*Levante\s+No\.?\s*([0-9]{8,15})", chunk_texto, re.IGNORECASE
  )
  if m_lev_box:
  ...
  return {
      ...
      "No. Bultos": no_bultos,
      "Valor Moneda Negociación (Casilla 76)": valor_casilla_76,
      "Valor Fletes/Seguros/Otros (Casilla 77)": valor_casilla_77,
...
def procesar_archivos(uploaded_files, progress_callback=None) -> pd.DataFrame:
  ...
  if "Número de formulario" in df.columns:
    df["Número de formulario"] = (
        df["Número de formulario"].astype(str).str.strip()
    )
    df = df[
    ...
    df = df.drop_duplicates(
        subset=["Número de formulario"], keep="last"
...
HEADER_FILL = PatternFill(
    start_color="1B4D3E", end_color="1B4D3E", fill_type="solid"
)
HEADER_FONT = Font(color="FFFFFF", bold=True)
TOTAL_FILL = PatternFill(
    start_color="D9E1D9", end_color="D9E1D9", fill_type="solid"
)
THIN_BORDER = Border(
...
def generar_excel(df: pd.DataFrame) -> bytes:
  ...
  if not df_export.empty:
    ...
    total_row["Sumatoria Fletes/Seguros/Otros (USD)"] = df_export[
        "Sumatoria Fletes/Seguros/Otros (USD)"
    ].sum()
    total_row["Valor Moneda Negociación (Casilla 76)"] = df_export[
        "Valor Moneda Negociación (Casilla 76)"
    ].sum()
    total_row["Valor Fletes/Seguros/Otros (Casilla 77)"] = df_export[
        "Valor Fletes/Seguros/Otros (Casilla 77)"
    ].sum()
  ...
  with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
    ...
    for col_idx, columna in enumerate(df_export.columns, start=1):
      longitudes = [len(str(columna))] + [
          len(str(v)) for v in df_export[columna].astype(str).tolist()
      ]
...
with col_titulo:
  ...
  st.markdown(
      "**Procesador Masivo de Declaraciones de Importación — Formulario 500**"
  )
...
with st.container():
  ...
  uploaded_files = st.file_uploader(
      "Arrastra y suelta tus archivos PDF individuales o paquetes comprimidos"
      " en formato .ZIP",
      type=["pdf", "zip"],
  ...
  with col_btn1:
    procesar = st.button(
        "🚀 Procesar Documentos", type="primary", use_container_width=True
    )
  with col_btn2:
...
if procesar:
  if not uploaded_files:
    st.warning(
        "Por favor carga al menos un archivo PDF o ZIP antes de ejecutar el"
        " procesamiento."
    )
  else:
    ...
    st.success(
        f"✅ Extracción exitosa. Se han consolidado {len(df)} declaración(es)"
        " única(s)."
    )
...
if not df.empty:
  ...
  busqueda = st.text_input(
      "🔍 Búsqueda rápida en el consolidado (Formulario, NIT, Importador,"
      " Manifiesto...)",
      "",
  )
  ...
  st.dataframe(
      ...
      column_config={
          ...
          "Sumatoria Fletes/Seguros/Otros (USD)": st.column_config.NumberColumn(
              format="%.2f"
          ),
          "Valor Moneda Negociación (Casilla 76)": (
  ...
  if not faltantes_totales.empty:
    with st.expander(
        f"⚠️ {len(faltantes_totales)} declaración(es) requieren revisión manual"
        " de campos"
    ):
      st.dataframe(
          faltantes_totales[
              ["Número de formulario", "Archivo", "Campos_no_encontrados"]
          ],
          use_container_width=True,
      )
  ...
  with st.expander("👁️ Inspección detallada por Formulario"):
    ...
    seleccion = st.selectbox(
        "Selecciona una declaración para ver su estructura completa", opciones
    )
  ...
  st.info(
      "Carga tus documentos aduaneros para activar el panel de análisis y las"
